...
if [ "$TARGET_ENV" == "prd" ]; then
  ...
  celery -A green_detective worker --beat --scheduler django_celery_beat.schedulers:DatabaseScheduler --loglevel=info -Q gd_general,gd_scrape,gd_pre_staging,gd_post_staging
else
